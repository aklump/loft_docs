...
function load_config() {
  ...
  docs_help_outline=$(find $docs_source_dir -name outline.json)
  
  # If it's not there we'll try to generate from a .ini file.
  if [[ ! "$docs_help_outline" ]]; then
    # Ini file
    docs_help_ini=$(find $docs_source_dir -name *.ini)

    if [[ "$docs_help_ini" ]]; then
      # Convert this to $docs_autogenerated_outline
      $docs_php "$CORE/includes/ini_to_json.php" "$docs_help_ini" "$docs_source_dir/$docs_autogenerated_outline"
      docs_help_outline="$docs_source_dir/$docs_autogenerated_outline"
  ...
  if [[ ! "$docs_help_output" ]]; then
    # Create $docs_autogenerated_outline from the file contents
    $docs_php "$CORE/includes/files_to_json.php" "$docs_source_dir" "$docs_source_dir/$docs_autogenerated_outline"

    docs_help_outline="source/$docs_autogenerated_outline"
  fi
...
$docs_php "$CORE/includes/page_vars.php" "$docs_help_outline" "index" "$get_version_return" >> "$docs_kit_dir/index.kit"
$docs_php "$CORE/includes/home.php" "$docs_help_outline" "$docs_tpl_dir" >> "$docs_kit_dir/index.kit"
...
for file in $docs_tmp_dir/*.html; do
  ...
  then
    ...
    $docs_php "$CORE/includes/page_vars.php" "$docs_help_outline" "$basename"  "$get_version_return" >> "$docs_kit_dir/$tmp_file"
