...
for file in $docs_source_dir/*; do
  if [ -f "$file" ]; then
    ...
    rsync -a "$docs_source_dir/$basename/" "$docs_drupal_dir/$basename/"
    rsync -a "$docs_source_dir/$basename/" "$docs_website_dir/$basename/"
    rsync -a "$docs_source_dir/$basename/" "$docs_html_dir/$basename/"
  fi
done
